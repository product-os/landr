...
module.exports = [
  ...
  // No users on pages other than the main one
  (combination, routes, path) => {
    return path.length === 0 || !combination.some((element) => {
  ...
  // No contributors on pages other than the main one
  (combination, routes, path) => {
    return path.length === 0 || !combination.some((element) => {
  ...
  // No highlights on pages other than the main one
  (combination, routes, path) => {
    return path.length === 0 || !combination.some((element) => {
  ...
  // No faq on pages other than the main one
  (combination, routes, path) => {
    return path.length === 0 || !combination.some((element) => {
