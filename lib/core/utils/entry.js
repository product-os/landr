...
if (typeof document !== 'undefined') {
  ...
  const locals = JSON.parse(
    document.getElementById('__LANDR__LOCALS').innerHTML
  )
  const routes = JSON.parse(
    document.getElementById('__LANDR__ROUTES').innerHTML
  )
  const mount = document.getElementById('mount')
  const AppRouter = () => {
    return (
      <BrowserRouter basename={__PATH_PREFIX__}>
...
const Entry = function render({ path, assets, locals, pages }) {
  let context = {}
  const App = (
