...
module.exports = async ({
  ...
  if (state.locals.git.token) {
    github.authenticate({
      type: 'token',
      token: state.git.token
    })
  }
  ...
  try {
    ...
    data = require(`${process.env.HOME}/landr/cache/${ghConfig.owner}/${ghConfig.name}.json`)
    console.log(data)
    actions.addLocals(data)
  }

  // if request went well write it to cache
  await fs.writeAsync(`${process.env.HOME}/landr/cache/${ghConfig.owner}/${ghConfig.name}.json`, JSON.stringify(data, null, 2))
  actions.addLocals(data)
}
