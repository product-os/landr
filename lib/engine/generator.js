 ...
 * @returns {Object[]} component combinations
 */
exports.getCombinations = (components, metadata, routes, path, theme) => {
  ...
  for (const product of generatorics.cartesian(...products)) {
    ...
    for (const permutation of permutations) {
      ...
      result.push(cloner(permutation))
    }
  }
...
exports.filterCombinations = (combinations, rules, routes, path) => {
  return combinations.filter((combination) => {
    ...
    return !scoring.isExtraCombination(
      combinations.slice(0, index), combination)
  })
}
