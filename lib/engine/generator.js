...
exports.getCombinations = function *(components, metadata, routes, path, options, rules) {
  const products = components.reduce((accumulator, component) => {
    const variants = ui.getVariants(
      ...
      const rulesBypage = rules.filter((rule) => {
        if (isEmpty(rule.pages)) {
          return true
        }

        // At least one path from the rules, should match the active one
        return rule.pages.some(page => page === path.join('/'))
      })

      const matchingRules = rulesBypage.filter((rule) => {
