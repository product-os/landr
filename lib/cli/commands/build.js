...
class BuildCommand extends Command {
  async run () {
    ...
    if (flags.output) {
      outputDir = path.isAbsolute(flags.output) ? flags.output : path.resolve(process.cwd(), flags.output)
    }
    ...
    if (flags.meta) {
      contractPath = path.isAbsolute(flags.meta) ? flags.meta : path.resolve(process.cwd(), flags.meta)
    ...
    if (flags.deploy && !NETLIFY_TOKEN) {
      throw new Error('Cannot deploy without a NETLIFY_AUTH_TOKEN. Did you forget to set an env var?')
    }
...
BuildCommand.flags = {
  meta: flags.string({
    char: 'm',
    description: 'The location of the meta file used to generate your landr site'
  }),
  output: flags.string({
    char: 'o',
