...
exports.create = async (metadata, templateDirectory, destinationDirectory) => {
  ...
  for (const file of files) {
    const resolvedPath = path.resolve(templateDirectory, file)
    ...
    const result = USE_TEMPLATES.includes(mime)
    ...
    await new Promise((resolve, reject) => {
      fs.writeFile(path.resolve(destinationDirectory, file), result, (error) => {
        if (error) {
          return reject(error)
        }
