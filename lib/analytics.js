...
class InvalidAnalyticsProvider extends TypedError {}
 ...
 */
export default (site, options) => {
  if (!options) {
    return NOOP_INSTANCE
  }

  if (INSTANCES[options.type] && INSTANCES[options.type][options.token]) {
  ...
  if (options.type === 'mixpanel') {
    ...
    Reflect.deleteProperty(require.cache, require.resolve('mixpanel-browser'))
  ...
  throw new InvalidAnalyticsProvider(
    `Unknown analytics provider: ${options.type}`
  )
}

export {
  InvalidAnalyticsProvider
}
